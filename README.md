...
### Установка
Для того чтобы начать пользоваться IPSpeed helper - достаточно скопировать, и вставить в терминал - следующие несколько команд:
...
wget -q -O /opt/bin/ipsh https://raw.githubusercontent.com/rino-soft-lab/ipsh/refs/heads/main/ipsh.sh
chmod +x /opt/bin/prs /opt/bin/ipsh
prs -t "sstp" "" "ipsh"
...
- Строка 3: Скачивание скрипта IPSh
...
> и нажать ввод. Скрипт проверит - все ли компоненты установились и (при необходимости) скорректирует настройки, после чего - будет запущен IPSpeed.info helper:
